...
async function ytgSetDownloadURL(stream) {
	...
	if (typeof window.ytgWinDecipher !== "function")
		await ytgUpdateDecipherInstance(html);
	const params = ytgGetParams(stream.signatureCipher || stream.cipher);
	const url = new URL(params.url);
	url.searchParams.set(
		params.sp ? params.sp : "signature",
		ytgDecipher(params.s)
	);
	return url.toString();
}
...
async function ytgAddButtonsToPlayerControls() {
	...
	const streams = config.streamingData.adaptiveFormats.filter((format) => {
		const mustInclude =
			...
			[1080, 720, 480].includes(format.height) &&
			!includedControl.includes(format.height);
	...
	select.onchange = async () => {
		...
		const videoFileName = `${config.videoDetails.title}-${
			ytgIsVideo(selectedStream)
				? "video" + selectedStream.qualityLabel
				: "audio"
		}-${selectedStream.bitrate}kbps.${ytgGetFileExtension(selectedStream)}`;
		const thumbnail =
	...
	allStreams.forEach((stream, i) => {
		...
		if (ytgIsVideo(stream))
			option.innerHTML =
				...
				stream.fps +
				"fps, " +
				ytgGetReadableBitrate(stream);
		else
			option.innerHTML =
				ytgGetType(stream) + " - " + ytgGetReadableBitrate(stream);
		select.appendChild(option);
	});
...
async function ytgUpdateDecipherInstance(html) {
	...
	const functionName = ytgBetweenText(
		jsContent,
		`a.set("alr","yes");c&&(c=`,
		`(decodeURIC`
	);
	const toEval = jsContent
		.replace(`${functionName}=function(a)`, `window.ytgWinDecipher=function(a)`)
		.replace("_yt_player", "grapsPlayer");
	eval(toEval);
}
